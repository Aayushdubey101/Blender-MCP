...
from blender_mcp.plugins import (
    BlenderMCPPlugin,
    discover_plugins,
    list_plugins_text,
    load_plugins,
...
class TestProtocol:
    def test_good_plugin_satisfies_protocol(self):
        assert isinstance(GoodPlugin(), BlenderMCPPlugin)

    def test_bad_shape_plugin_does_not(self):
        assert not isinstance(BadShapePlugin(), BlenderMCPPlugin)


class TestDiscoverPlugins:
    def test_returns_empty_when_none_installed(self):
        with patch("blender_mcp.plugins._iter_entry_points", return_value=[]):
    ...
    def test_filters_out_bad_shape_plugins(self, caplog):
        ...
        with patch("blender_mcp.plugins._iter_entry_points", return_value=eps):
            with caplog.at_level("ERROR", logger="blender_mcp.plugins"):
                result = discover_plugins()
        assert result == [good]
        assert "does not satisfy BlenderMCPPlugin protocol" in caplog.text
...
class TestLoadPlugins:
    ...
    def test_calls_register_with_correct_args(self):
        good = GoodPlugin()
        with patch("blender_mcp.plugins._iter_entry_points", return_value=[_ep("good", good)]):
...
class TestServerWiresLoader:
    ...
    def test_server_module_exposes_loaded_plugins(self):
        ...
        import blender_mcp.server as server  # noqa: PLC0415
